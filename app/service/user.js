...
module.exports = app => {
  class MonthServer extends app.Service {
    ...
    * login(username, password) {
      const md5Password = crypto.createHash('md5').update(password).digest('hex');
      const articles = yield app.mysql.query('select id, nick, phone, email, pic, username, registry_time, default_address_id from design_user where username = ? or phone = ? and password = ?', [ username,username, md5Password ]);
      return articles[0];
    }
    // 获取文章列表
    // 获取文章列表

(function($){ 
	var functions = {
		init:function(input, step, max, min, digit){
			...
			//添加左右加减号
			input.wrap("<div style = 'overflow:hidden;width:" + (width -3) + "px;height:" + height + "px;border: 1px solid #CCC;'></div>");
			input.before("<div class=\"number-input-left\" id = '" + input.attr('id') + "l'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-right: 1px solid #CCC;float:left'>-</div>");
			input.after("<div class=\"number-input-right\" id = '" + input.attr('id') + "r'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-left: 1px solid #CCC;float:right'> + </div>");
			//左右调用执行
